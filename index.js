        ...
        return new Promise(function(resolve, reject) {
            ...
            function gracefulHealthCheck(healthUrl, check) {
                return function(req, res, next) {
                    ...
                    if (isStopping) {
                        debug(
                            'HealthCheck(%s), server is shutting-down.',
                            healthUrl
                        );
                        res.statusCode = 503;
                        res.setHeader('Content-Type', 'application/json');
            ...
            if (
                ...
                Object.keys(options.healthChecks).map(key => {
                    server.get(
                        key,
                        gracefulHealthCheck(key, options.healthChecks[key])
                    );
                });
            }
            ...
            function gracefulCleanup(signal) {
                return function() {
                    ...
                    if (!isStopping) {
                        isStopping = true;
                        debug(
                            'Server received %s interrupt, starting cleanup.',
                            signal
                        );
